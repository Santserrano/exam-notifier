...
describe('Middleware Configuration', () => {
  it('should use json middleware', async () => {
    ...
    jest.doMock('express', () => {
      const actualExpress = jest.requireActual('express') as typeof import('express');
  ...
  describe('Routes', () => {
    ...
    it('should register notification routes', async () => {
      const { app } = await import('../index');
      expect(app.use).toHaveBeenCalledWith('/api/diaries/notificaciones', expect.any(Object));
    });
  });
});