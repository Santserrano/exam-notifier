...
describe('NotificationService', () => {
  ...
  it('getConfigByProfesor - config no existe, crea config', async () => {
    ...
    mockPrisma.notificacionConfig.create.mockResolvedValue({ webPushEnabled: true });
    const config = await notificacionService.getConfigByProfesor('prof1');
  ...
  it('getConfigByProfesor - config existe', async () => {
    mockPrisma.profesor.findUnique.mockResolvedValue({ id: 'prof1' });
    mockPrisma.notificacionConfig.findUnique.mockResolvedValue({ webPushEnabled: true });
    const config = await notificacionService.getConfigByProfesor('prof1');
    expect(config.webPushEnabled).toBe(true);
  });
  ...
  it('updateConfig - actualiza config', async () => {
    mockPrisma.notificacionConfig.upsert.mockResolvedValue({ webPushEnabled: true });
    const result = await notificacionService.updateConfig('prof1', { webPushEnabled: true, emailEnabled: false, smsEnabled: false, avisoPrevioHoras: 24 });
  ...
  it('updateConfig - error', async () => {
    ...
    await expect(
      notificacionService.updateConfig('prof1', { webPushEnabled: true, emailEnabled: false, smsEnabled: false, avisoPrevioHoras: 24 })
  ...
  it('saveWebPushSubscription - actualiza existente', async () => {
    ...
    mockPrisma.webPushSubscription.update.mockResolvedValue({ id: 'sub1', endpoint: 'e' });
  ...
  it('saveWebPushSubscription - crea nueva', async () => {
    ...
    mockPrisma.webPushSubscription.create.mockResolvedValue({ id: 'sub2', endpoint: 'e' });
    const result = await notificacionService.saveWebPushSubscription('prof1', {
      endpoint: 'e',
      keys: { p256dh: 'p', auth: 'a' },
    });
  ...
  it('saveWebPushSubscription - error', async () => {
    mockPrisma.webPushSubscription.findFirst.mockRejectedValue(new Error('fail'));
    await expect(
  ...
  it('getWebPushSubscriptions - error', async () => {
    mockPrisma.webPushSubscription.findMany.mockRejectedValue(new Error('fail'));
    await expect(notificacionService.getWebPushSubscriptions('prof1')).rejects.toThrow('Error al obtener las suscripciones');
  });
  ...
  it('deleteWebPushSubscription - error', async () => {
    ...
    await expect(notificacionService.deleteWebPushSubscription('sub1')).rejects.toThrow('Error al eliminar la suscripción');
  });
});