...
describe('sendPushToProfesor', () => {
  ...
  it('elimina subscripción inválida (statusCode 410)', async () => {
    ...
    expect(notificacionService.deleteWebPushSubscription).toHaveBeenCalledWith(1);
  });

  it('cubre el error global (catch)', async () => {
    (notificacionService.getWebPushSubscriptions as jest.Mock).mockRejectedValue(new Error('fail'));
    await expect(sendPushToProfesor('prof1', 'titulo', 'cuerpo')).rejects.toThrow('fail');
  });
});
